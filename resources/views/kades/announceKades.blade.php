...
<div class="body">
	...
	<div class="atas" style="padding-bottom: 10px;">
		<form class="form-inline" method="POST" action="/kades/{{$us->id}}/pengumuman">
			{{csrf_field()}}
			<div class="cari">
				<input class="form-control" name="cari" type="text" id="myInput" onkeyup="" placeholder="cari judul..." value="<?php echo isset($_POST['cari']) ? $_POST['cari'] : '' ?>">
				<button class="btn btn-default" type="submit" name="submit"><span class="glyphicon glyphicon-search"></span></button>
			</div>
		</form>
		
		<div class="add">
			<a href="/kades/{{$us->id}}/pengumuman/baru" class="btn btn-warning"><span class="glyphicon glyphicon-plus"></span>	Buat Pengumuman</a>
		</div>
	</div>
				...
				<div class="panel panel-primary">
					<div class="panel-heading">
						<label><h4 class="panel-title">
							<a href="/kades/{{$us->id}}/pengumuman/{{$key->id}}" style="">{{$key->judul}}</a></h4>
						</label>
						<span class="tanda">
							<?php if ($data > 0): ?>
					...
					<a href="/kades/{{$us->id}}/pengumuman/{{$key->id}}" id="link">
						<label id="wadah">
					...
					<tbody>
			            ...
			            foreach ($ann as $key): ?>
			            	<?php  $no = $no + 1; ?>
					            <tr>
					            	<td><?php echo $no ?></td>
					            	<td><a href="/kades/{{$us->id}}/pengumuman/{{$key->id}}" style="text-decoration: none;">{{$key->judul}}</a></td>
